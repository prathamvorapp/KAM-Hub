import { BrandWithKAM, PriceRecord, RevenueRecord, Milestone, DepartmentMetrics, BrandMetrics, TimelineData } from './types'
...
export class MilestoneGenerator {
  ...
  generateBrandTimeline(brand: BrandWithKAM): TimelineData {
    ...
    console.log('Generating monthly milestones:', {
    ...
    while (currentDate <= endDate) {
      ...
      if (!hasExistingMilestone) {
        ...
        console.log('Skipping month (already has milestone):', this.formatMonthLabel(currentDate))
      }
    ...
    console.log('Brand Timeline Milestones:', {
      brandId: brand.kam_assignment?.brand_uid,
      brandName: brand.kam_assignment?.brand_name,
  ...
  private calculateBrandMetrics(
    ...
    // Debug: Log June 2026 specifically
    if (targetMonth.getFullYear() === 2026 && targetMonth.getMonth() === 5) {
      console.log('June 2026 Brand Metrics:', {
        brandName: brand.kam_assignment?.brand_name,
