...
export class DataQueryEngine {
  ...
  getExpenseByKAM(kamName: string): QueryResult {
    try {
      console.log(`🔍 Searching for KAM: "${kamName}"`);
      console.log(`📊 Total expense records: ${this.expenseData.length}`);
  ...
  private getAvailableKAMs(): string[] {
    ...
    console.log(`📋 All unique KAMs in Expense.csv (${allKAMs.length}):`, allKAMs);
