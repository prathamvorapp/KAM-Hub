/**
 * API Client - Uses Next.js API routes (Supabase-backed)
 * This file contains functions for making fetch calls to our Next.js API routes.
 */

export const apiClient = {
  // User Profile
  getUserProfile: async (email: string) => { // This email is used to query specific profile-by-email endpoint
    try {
  ...
  // Churn Analytics
  getChurnAnalytics: async () => { // Removed email parameter
    try {
  ...
  getAgentResponseTimeDetails: async (agentName: string) => { // Removed userEmail parameter
    try {
      // NOTE: This client-side filtering logic is inefficient.
      // Ideally, the /api/churn endpoint should support filtering by agentName server-side.
      const response = await fetch(`/api/churn?limit=10000`, { // Fetch all accessible churn data
        credentials: 'include'
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
  ...
  recordCallAttempt: async (params: {
    ...
    try {
      const response = await fetch(`/api/follow-up/${params.rid}/attempt`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ // Removed email from body
          rid: params.rid,
  ...
  getFollowUpStatus: async (rid: string, churnReason?: string) => { // Removed email parameter
    try {
  ...
  // Churn Data
  getChurnData: async (params: { page?: number; limit?: number; search?: string; filter?: string }): Promise<any> => { // Removed email parameter
    try {
  ...
  // Churn Statistics
  getChurnStatistics: async () => { // Removed email parameter
    try {
  ...
  getApprovals: async () => { // Removed email parameter
    try {
  ...
  // Get visits
  getVisits: async (params: { search?: string; page?: number; limit?: number; paginationOpts?: { numItems?: number } }) => { // Removed email parameter
    try {
      const limit = params.paginationOpts?.numItems || params.limit || 999999;
      const searchParams = new URLSearchParams({
  ...
  getVisitStatistics: async (bustCache = false) => { // Removed email parameter
    try {
      const url = `/api/data/visits/statistics${bustCache ? '?bustCache=true' : ''}`; // Removed email from URL
  ...
  // Get MOMs
  getMOM: async (params: { search?: string; page?: number; limit?: number }) => { // Removed email parameter
    try {
      const searchParams = new URLSearchParams({
        page: (params.page || 1).toString(),
  ...
  // Get MOMs (alias)
  getMOMs: async (params: { search?: string; page?: number; limit?: number }) => { // Removed email parameter
    try {
      const searchParams = new URLSearchParams({
        page: (params.page || 1).toString(),
        limit: (params.limit || 1000).toString()
  ...
  approveVisit: async (visitId: string, approverEmail: string, approvalStatus: string, rejectionRemarks?: string) => { // approverEmail stays, as it's part of the data
    try {
  ...
  getMasterData: async (page?: number, limit?: number, search?: string) => { // Removed email parameter
    try {
      const searchParams = new URLSearchParams({
  ...
  getBrandsByAgentEmail: async (email: string, page?: number, limit?: number, search?: string) => { // This email is used to query specific profile-by-email endpoint
    try {
      const searchParams = new URLSearchParams({
        page: (page || 1).toString(),
        limit: (limit || 10000).toString() // High default to get all brands
      });
      if (search) searchParams.append('search', search);
      
      const queryString = searchParams.toString();
      const url = `/api/data/master-data/brands/${encodeURIComponent(email)}?${queryString}`; // email is part of the URL path here
      
      const response = await fetch(url, {
        credentials: 'include' // Ensure cookies are sent for authentication
      });
      if (!response.ok) {
        console.error(`❌ getBrandsByAgentEmail failed: HTTP ${response.status}`);
  ...
  getOverdueFollowUps: async (kam?: string) => { // Removed email parameter
    try {
      const searchParams = new URLSearchParams();
      if (kam) searchParams.append('kam', kam);
      
      const response = await fetch(`/api/follow-up/reminders/overdue?${searchParams.toString()}`, {
        credentials: 'include' // Ensure cookies are sent
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return await response.json();
  ...
  getActiveFollowUps: async (kam?: string) => { // Removed email parameter
    try {
      const searchParams = new URLSearchParams();
      if (kam) searchParams.append('kam', kam);

      const response = await fetch(`/api/follow-up/reminders/active?${searchParams.toString()}`, {
        credentials: 'include' // Ensure cookies are sent
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return await response.json();
  ...
  resubmitMoM: async (visitId: string) => { // Removed agentEmail parameter
    try {
      const response = await fetch(`/api/data/visits/${visitId}/resubmit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({}) // No agent_email needed in body, it's from session
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return await response.json();
    } catch (error) {
  ...
  // Reschedule visit
  rescheduleVisit: async (visitId: string, newDate: string, reason: string) => { // Removed rescheduledBy parameter
    try {
      const response = await fetch(`/api/data/visits/${visitId}/reschedule`, {
        ...
        body: JSON.stringify({
          ...
          reason,
          // rescheduled_by: rescheduledBy // Removed from body
        })
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return await response.json();
  ...
  // Demo operations
  getDemosForAgent: async (agentId: string, role?: string, teamName?: string) => { // Removed role, teamName as they should come from auth
    try {
      const searchParams = new URLSearchParams();
      if (agentId) searchParams.append('agentId', agentId);
      // No need to pass role or teamName, server will derive from auth
      
      const response = await fetch(`/api/data/demos?${searchParams.toString()}`, {
        credentials: 'include'
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return await response.json();
  ...
  rescheduleDemo: async (demoId: string, scheduledDate: string, scheduledTime: string, reason: string) => { // Removed userEmail, userRole
    try {
      const response = await fetch(`/api/data/demos/${demoId}/schedule`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  getDemoStatistics: async (agentId?: string) => { // Removed teamName, role
    try {
      const searchParams = new URLSearchParams();
      if (agentId) searchParams.append('agentId', agentId);

      const response = await fetch(`/api/data/demos/statistics?${searchParams.toString()}`, {
        credentials: 'include'
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return await response.json();
  ...
  getTeamVisitStatistics: async () => { // Removed email parameter
    try {
