...
import { useBrands, usePrices, useRevenueRecords, useDataContext, useChurnRecords, usePriceData, useExpenseRecords } from '@/lib/di-data-context'
import { TimelineSkeleton } from '@/components/di/TimelineSkeleton'
import { BrandWithKAM, RevenueRecord } from '@/lib/di-types'
import { calculateSwitchingCostIndex, BrandSCI } from '@/lib/di-switching-cost-calculator'
import { ChurnAnalysis } from '@/components/di/ChurnAnalysis'
import { ExpenseAnalysis } from '@/components/di/ExpenseAnalysis'
import Link from 'next/link'
import { useMemo, useState } from 'react'

// Format rupee amount to Lakhs or Crores
                  ...
                  return Array.from(kamMap.entries())
                    ...
                    .map(([kam, data]) => (
                      <tr key={kam} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold text-green-600">{fmtRevenue(data.revenue)}</td>
                      </tr>
                    ))
                })()}
              </tbody>
                  ...
                  return (
                    <tr key={brand.restaurant_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold text-blue-600">
                        {fmtRevenue(brandRevenue.posRevenue)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold text-green-600">
                        {fmtRevenue(brandRevenue.totalRevenue)}
                      </td>
