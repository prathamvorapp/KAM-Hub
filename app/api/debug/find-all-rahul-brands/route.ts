...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Search variations
    const searches = {
      exact_email: records.filter((r: any) => r.kam_email_id === 'rahul.taak@petpooja.com'),
      email_lowercase: records.filter((r: any) => r.kam_email_id?.toLowerCase() === 'rahul.taak@petpooja.com'),
      email_contains_rahul_taak: records.filter((r: any) => r.kam_email_id?.toLowerCase().includes('rahul.taak')),
      email_contains_rahul: records.filter((r: any) => r.kam_email_id?.toLowerCase().includes('rahul')),
      name_exact: records.filter((r: any) => r.kam_name === 'Rahul Taak'),
      name_lowercase: records.filter((r: any) => r.kam_name?.toLowerCase() === 'rahul taak'),
      name_contains_rahul: records.filter((r: any) => r.kam_name?.toLowerCase().includes('rahul')),
      secondary_email: records.filter((r: any) => r.kam_email_id_secondary?.toLowerCase().includes('rahul.taak')),
      secondary_name: records.filter((r: any) => r.kam_name_secondary?.toLowerCase().includes('rahul')),
    };
    
    // Get unique brands across all searches
    const allRahulBrands = new Set();
    Object.values(searches).forEach(results => {
      results.forEach((r: any) => allRahulBrands.add(r.brand_name));
    });
    
    // Check for any column that might contain rahul.taak
    const anyFieldContainsRahul = records.filter((r: any) => {
