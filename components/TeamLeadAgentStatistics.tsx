...
export default function TeamLeadAgentStatistics({ userEmail, onRefresh }: TeamLeadAgentStatisticsProps) {
  ...
  const filteredAndSortedAgents = teamAgentStatistics
    .filter(agent => {
  ...
  // Calculate team metrics (excluding team lead)
  const teamMetrics = teamAgentStatistics.reduce((metrics, agent) => {
    if (!agent.error) {
  ...
  const teamProgress = teamMetrics.totalMonthlyTarget > 0 
    ? (teamMetrics.totalCurrentMonthCompleted / teamMetrics.totalMonthlyTarget) * 100 
    : 0

  // Performance rankings
  const getTeamRank = (agent: AgentStatistics) => {
    const activeAgents = teamAgentStatistics.filter(a => !a.error)
    ...
    <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 mb-6 border border-blue-100">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-4">
          ...
          <div className="text-center p-3 bg-white rounded-lg shadow-sm">
            <div className="text-2xl font-bold text-green-600">{teamMetrics.totalVisitsDone}</div>
            <div className="text-xs text-gray-600">Visits Done</div>
          </div>
          <div className="text-center p-3 bg-white rounded-lg shadow-sm">
            <div className="text-2xl font-bold text-yellow-600">{teamMetrics.totalVisitsPending}</div>
            <div className="text-xs text-gray-600">Pending</div>
          </div>
          <div className="text-center p-3 bg-white rounded-lg shadow-sm">
            <div className="text-2xl font-bold text-orange-600">{teamMetrics.totalCurrentMonthCompleted}/{teamMetrics.totalMonthlyTarget}</div>
          ...
          <div className="text-center p-3 bg-white rounded-lg shadow-sm">
            <div className="text-2xl font-bold text-red-500">{teamMetrics.agentsNeedingAttention}</div>
      ...
      <div className="flex flex-wrap items-center justify-between mb-6 gap-4">
        <div className="flex items-center space-x-4">
          ...
          <label className="flex items-center text-sm">
            <input
              id="show-active-team-agents"
              name="show-active-team-agents"
              type="checkbox"
        ...
        <div className="text-sm text-gray-600">
          Showing {filteredAndSortedAgents.length} of {teamAgentStatistics.length} team members
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredAndSortedAgents.map((agent, index) => {
                ...
                const teamRank = getTeamRank(agent)
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {!agent.error && (
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          ...
                          teamRank <= 3 ? 'bg-green-100 text-green-800' :
                          teamRank <= 5 ? 'bg-blue-100 text-blue-800' :
                          'bg-gray-100 text-gray-800'
                        }`}>
                          #{teamRank} {teamRank === 1 ? '👑' : teamRank <= 3 ? '🏆' : ''}
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  .map((agent, index) => (
                    <tr key={index} className={agent.error ? 'bg-red-50' : 'hover:bg-gray-50'}>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="w-6 h-6 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-full flex items-center justify-center mr-2">
                            <span className="text-xs font-bold text-white">
                              {agent.agent_name.split(' ').map(n => n[0]).join('').toUpperCase()}
                            </span>
                          </div>
